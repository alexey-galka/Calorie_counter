...
const $ageInput = document.forms.counter.elements.age;
const $heightInput = document.forms.counter.elements.height;
const $weightInput = document.forms.counter.elements.weight;
const $sex = document.forms.counter.elements.gender;
const $results = document.querySelector(".counter__result");
const $activity = document.forms.counter.elements.activity;
...
$countButton.addEventListener("click", () => {
  ...
  $caloriesNormal.textContent = calcNormalWeight($sex);
  $caloriesMin.textContent = getLoseWeight(calcNormalWeight($sex));
  $caloriesMax.textContent = getGainWight(calcNormalWeight($sex));
});
...
function calcNormalWeight(sex) {
  const COUNT =
    10 * $weightInput.value + 6.25 * $heightInput.value - 5 * $ageInput.value;
  if (sex === "female") {
    return ((COUNT - 161) * getActivity($activity.value)).toFixed();
  }
  return ((COUNT + 5) * getActivity($activity.value)).toFixed();
}
// End of weight maintenance

// Start of gain and lose weight funcs
function getGainWight(func) {
  return (func * 1.15).toFixed();
}

function getLoseWeight(func) {
  return (func * 0.85).toFixed();
}
